...
def plot_decision_boundary(clf, X_train: np.ndarray, X_test: np.ndarray,
    ...
    # creating limits in graph -> class. model dont touch axes
    x_min, x_max = X_train[:, 0].min() - 1, X_train[:, 0].max() + 1
    y_min, y_max = X_train[:, 1].min() - 1, X_train[:, 1].max() + 1
    # rectangular mesh with length x_max - x_min and breadth y_max - y_max with step size h
    ...
    plt.scatter(X_train[:, 0], X_train[:, 1], c=y_train, cmap=cmap_bold, marker="x", s=25, label="Training Points")
    plt.scatter(X_test[:, 0], X_test[:, 1], c=y_test, cmap=cmap_bold, marker="^", edgecolor="k", s=25, label="Test Points")
