...
def fit_gradient_boosting_regressor(X: np.ndarray, n_samples: int, y: np.ndarray,
    ...
    model = GradientBoostingRegressor(loss=loss, n_estimators=n_samples, learning_rate=learning_rate, max_depth=max_depth, random_state=random_state)
    model.fit(X, y)
    # possibility:
    model.predict(X)
    # wrong: error = accuracy_score(y_true = y, y_pred=y_p) -> no regression
    # error1 = ((y - y_p) ** 2) different error values
    # error1 = model.score(X, y) 1 error value
    # attribute: shape # of estimators
    error = model.train_score_
    return model, error
