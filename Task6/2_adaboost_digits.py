...
def get_acc_for_estimators(base_estimator: DecisionTreeClassifier, X_train: np.ndarray,
    ...
    acc_train = [accuracy_score(y_true=y_train, y_pred=y_pred) for y_pred in model.staged_predict(X_train)]
    acc_test = [accuracy_score(y_true=y_test, y_pred=y_pred) for y_pred in model.staged_predict(X_test)]
