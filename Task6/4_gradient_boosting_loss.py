...
def evaluate_loss_fn(X_train: np.ndarray, y_train: np.ndarray,
                     ...
                     loss_fn="squared_erorr", learning_rate=0.5) -> np.ndarray:
    """Fits a GradientBoostingRegressor on the training data and returns the training and test mse for all iterations"""
    model = GradientBoostingRegressor(loss=loss_fn, learning_rate=learning_rate, n_estimators=200, random_state=0)
...
def main():
    ...
    for loss_fn in tqdm(["squared_error", "lad", "huber", "quantile"]):
    ...
    for lr in [1 / 32, 1 / 16, 1 / 8, 1 / 4, 1 / 2, 1]:
        mses = evaluate_loss_fn(X_train, y_train, X_test, y_test, learning_rate=lr, loss_fn="squared_error")
