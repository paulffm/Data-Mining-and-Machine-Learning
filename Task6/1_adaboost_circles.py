...
def plot_dataset(X_train: np.ndarray, X_test: np.ndarray, y_train: np.ndarray, y_test: np.ndarray) -> None:
    ...
    plt.figure()
    plt.scatter(X_train[:, 0], X_train[:, 1], c=y_train,  cmap=ListedColormap(['b', 'r']), marker='o', label='training data')
    plt.scatter(X_test[:, 0], X_test[:, 1], c=y_test, cmap=ListedColormap(['b', 'r']), marker='x', label='test data')
    plt.title('Plot of test and training data')
    plt.xlabel("$X_0$")
    plt.ylabel("$X_1$")
    plt.legend()
    plt.show()
