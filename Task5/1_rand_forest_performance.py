...
def cross_val_dt_rt(load_fun) -> (dict, dict):
    ...
    # return_X_y returns only data and target object but with all samples
    X, y = load_fun(return_X_y=True)
    clf_dt = DecisionTreeClassifier()
    clf_rf = RandomForestClassifier(n_estimators=20)
    scores_dt = cross_validate(clf_dt, X, y, scoring="accuracy", cv=10, n_jobs=-1, return_train_score=True)
    scores_rf = cross_validate(clf_rf, X, y, scoring="accuracy", cv=10, n_jobs=-1, return_train_score=True)
...
def cross_val_dt_rf_continuous(load_fun) -> (dict, dict):
    ...
    scores_dt = cross_validate(clf_dt, X, y, cv=10, scoring='mean_squared_error', return_train_score=True)
    scores_rf = cross_validate(clf_rf, X, y, cv=10, scoring='mean_squared_error', return_train_score=True)
    return scores_dt, scores_rf
