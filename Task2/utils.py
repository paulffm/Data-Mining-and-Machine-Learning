...
def generate_dataset() -> (np.ndarray, np.ndarray):
    ...
    '''features teilen sich auf informative, redundante '''
...
def measure_accuracy(clf: sklearn.base.BaseEstimator, X_test: np.ndarray, X_train: np.ndarray,
    ...
    # estimator learning from data: estimator = estimator.fit(data, target)
    clf.fit(X_train, y_train)
    # estimator predicting from data: prediction = estimator.predict(data)
    y_train_pred = clf.predict(X_train)
    y_test_pred = clf.predict(X_test)
    # Eval: accuracy_score(y_true, y_pred): How many are right: like in function accuracy in Task 1
...
#verschiedene k-Werte, Datensatz, Zielvektor
#Genauigkeit für verschiedene k-Werte berechnen von Trainingsperformance und Testperformance
def evaluate(k: int, X: np.ndarray, y: np.ndarray) -> (float, float):
    ...
    # KNeighborsClassifier implementing the k-nearest neighbors vote -> our estimator
