...
def evaluate_metric(X: np.ndarray, y: np.ndarray, clf: sklearn.base.BaseEstimator, metric: staticmethod.__func__)\
    ...
    y_pred = cross_val_predict(clf, X, y, cv=5, n_jobs=-1, verbose=0, method="predict")
    # all error metrics take y_true, y_pred
...
def main():
    ...
    # Attention: RandomForestRegressor has no method = predict_proba, why?
